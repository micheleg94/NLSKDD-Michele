...
def main():
    ...
    for i in range(1,11):
        pathTrain='D:\Tesi\dataset\DatasetAgg\\CICIDS\\DS_'+str(i)+'\\Train_standard'
        pathTest = 'D:\Tesi\dataset\DatasetAgg\\CICIDS\\DS_1\\Test_standard6'
        ...
        if LOAD_AUTOENCODER==0:
            ...
            autoencoder.save('D:\Tesi\dataset\DatasetAgg\\CICIDS\\DS_'+str(i)+'\\EncoderCICIDS.h5')
        ...
        encoded_train.to_csv('D:\Tesi\dataset\DatasetAgg\\CICIDS\\DS_'+str(i)+'\Train_encoded.csv', index=False)
        
        for j in range(1,10):
            pathTest = 'D:\Tesi\dataset\DatasetAgg\\CICIDS\\DS_'+str(i)+'\\Test_standard'+str(j)
            test = pd.read_csv(pathTest + ".csv")
            train_X, train_Y, test_X, test_Y= getXY(train, test)
            ...
            encoded_test.to_csv('D:\Tesi\dataset\DatasetAgg\\CICIDS\\DS_'+str(i)+'\Test_encoded'+str(j)+'.csv', index=False)
