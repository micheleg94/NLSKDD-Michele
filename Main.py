...
def main():
    ...
    train_df=pd.read_csv(path+"\KDDTrain+aggregate.csv",delimiter=",")
    test_df=pd.read_csv(path+"\KDDTest+aggregate.csv",delimiter=",")
    ...
    train_df.to_csv(path+'\DatasetStand\Train_standard1.csv', index=False)
    #copyOfTrain = minMaxScale(copyOfTrain, numeric_cols)
    
    
    test_df.to_csv(path+'\DatasetStand\Test_standard1.csv', index=False)
